...
Quarkyll has been developed as a [Gem-based theme](http://jekyllrb.com/docs/themes/) for easier use, and is 100% compatible with GitHub Pages when used as a remote theme.

Check [jekyll-include-cache](https://github.com/benbalter/jekyll-include-cache) plugin
{: .notices .error}
...
**ProTip:** Be sure to remove `/docs` and `/test` if you forked Quarkyll. These folders contain documentation and test pages for the theme and you probably don't want them littering up your repo.
{: .notices .info}
...
4. Add `remote_theme: "drupix/jekyll-theme-quarkyll@0.1.0"` to your `_config.yml` file. Remove any other `theme:` or `remote_theme:` entry.

You may also optionally specify a branch, [tag](https://github.com/drupix/jekyll-theme-quarkyll/tags), or commit to use by appending an @ and the Git ref (e.g., `drupix/jekyll-theme-quarkyll@0.1.0` or `drupix/jekyll-theme-quarkyll@bbf3cbc5fd64a3e1885f3f99eb90ba92af84063d`). This is useful when rolling back to older versions of the theme. If you don't specify a Git ref, the latest on `master` will be used.
...
To do so fork the [Minimal Mistakes theme](https://github.com/drupix/jekyll-theme-quarkyll/fork), then rename the repo to **USERNAME.github.io** --- replacing **USERNAME** with your GitHub username.
...
If you forked or downloaded the `jekyll-theme-quarkyll` repo you can safely remove the following folders and files:
...
- `/test`
- `CHANGELOG.md`
- `jekyll-theme-quarkyll.gemspec`
- `README.md`
- `screenshot-layouts.png`
- `screenshot.png`

**Note:** If forking the theme be sure to update `Gemfile` as well. The one found at the root of the project is for building the theme's Ruby gem and is missing dependencies. To properly setup a [`Gemfile`](https://github.com/drupix/jekyll-theme-quarkyll/blob/master/docs/Gemfile) with the theme, consult the "[Install Dependencies](https://mmistakes.github.io/minimal-mistakes/docs/installation/#install-dependencies)" section.
{: .notice--warning}
...
**ProTip:** The source code and content files for this site can be found in the [`/docs` folder](https://github.com/drupix/jekyll-theme-quarkyll/tree/master/docs) if you want to copy or learn from them.
{: .notice--info}

### Starting Fresh

Starting with an empty folder and `Gemfile` you'll need to copy or re-create this [default `_config.yml`](https://github.com/drupix/jekyll-theme-quarkyll/blob/master/_config.yml) file. For a full explanation of every setting be sure to read the [**Configuration**]({{ "/docs/configuration/" | relative_url }}) section.
...
- [`_data/navigation.yml`][navigation.yml] - navigation [documentation]({{ "/docs/navigation/" | relative_url }})
  ...
  [ui-text.yml]: https://github.com/drupix/jekyll-theme-quarkyll/blob/master/_data/ui-text.yml
  [navigation.yml]: https://github.com/drupix/jekyll-theme-quarkyll/blob/master/_data/navigation.yml
...
- Replace `<site root>/index.md` with a modified [Minimal Mistakes `index.html`](https://github.com/drupix/jekyll-theme-quarkyll/blob/master/index.html). Be sure to enable pagination if using the [`home` layout]({{ "/docs/layouts/#home-page" | relative_url }}) by adding the necessary lines to **_config.yml**.
...
**Note:** When clearing out the `assets` folder be sure to leave any files you've added and need. This includes images, CSS, or JavaScript that aren't already [bundled in the theme](https://github.com/drupix/jekyll-theme-quarkyll/tree/master/assets).
{: .notice--warning}
...
Then run `bundle update` and add `theme: jekyll-theme-quarkyll` to your `_config.yml`.
