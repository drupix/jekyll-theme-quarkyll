...
1. Support for a theme `assets` folder was recently [added to Jekyll core](https://github.com/jekyll/jekyll/pull/5364), but has yet to be released or rolled into the `github-pages` gem. Meaning you can't use Quarkyll as a Ruby gem there just yet... locally served or self-hosted installs should be fine if you don't mind using a pre-release version of Jekyll.
...
- Replace `<site root>/index.html` with a modified [Quarkyll `index.html`](https://github.com/drupix/jekyll-theme-quarkyll/blob/master/index.html).
